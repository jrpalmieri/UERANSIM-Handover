...
`xnHandoverRequestSource()`: Process RRC HANDOVER_REQUEST message
...
`xnHandoverRequestAckSource()`: Process received Xn Handover Request Ack message from target gnb

    - check TX_N_RELOC_PREP timer not expired - if expired, discard message.
    - create GTP-U tunnel using XnTask's GTPTask.
...
`xnHandoverPreparationFailureSource()`:  Process received Xn Handover Preparation Failure message from target gnb

    - check TX_N_RELOC_PREP timer not expired - if expired, discard message.
    - send XnToRrc::HANDOVER_PREPARATION_FAILURE_RECEIVE msg to RRCTask.
    - Disable TX_N_RELOC_PREP timer.

`xnSnStatusTransferSource()`:  Process RRC SN_STATUS_TRANSFER message

    - create Xn SN Status Transfer message
    - send to Sctp task, directed at targetNCI

`xnUeContextReleaseSource()`: Process received Xn UE Context Release message from target gnb
...
`xnHandoverRequestTarget()`:  Process received Xn Handover Request from source gnb.

    - send XnToRrc::HANDOVER_REQUEST_RECEIVE to RRCtask.

`xnHandoverCancelTarget()`:  Process received Xn Handover Cancel from source gnb

    - send XnToRrc::HANDOVER_CANCEL_RECEIVE to RRCtask.

`xnHandoverRequestAckTarget()`:  Process RRC HANDOVER_REQUEST_ACK_SEND message

    - generate Xn Handover Request Ack message
    - send to Sctp task, directed at sourceNCI

`xnHandoverPreparationFailureTarget()`:  Process RRC HANDOVER_PREPARATION_FAILURE_SEND message

    - generate Xn Handover Preparation Failure message
    - send to Sctp task, directed at sourceNCI

`xnSnStatusTransferTarget()`: Process received Xn SN Status Transfer from source gnb:
