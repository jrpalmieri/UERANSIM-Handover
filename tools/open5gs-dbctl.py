...
def _normalize_imsi_from_supi(supi):
    if not supi:
        return ""
...
def _extract_expected_multi_profiles(yaml_file, count, imsi_override=None):
    try:
    ...
    if not isinstance(ue_config, dict):
        print("Error: UE YAML root must be a map/object")
        return None

    if imsi_override:
        base_imsi = _normalize_imsi_from_supi(imsi_override)
    else:
        base_imsi = _normalize_imsi_from_supi(ue_config.get('supi', ''))
    ...
    key = str(ue_config.get('key', '')).strip()
    amf = str(ue_config.get('amf', '8000')).strip()
    op_value = str(ue_config.get('op', '')).strip()
    op_type = str(ue_config.get('opType', 'OPC')).upper().strip()

    expected_opc = op_value if op_type == 'OPC' else None
    expected_op = op_value if op_type == 'OP' else None

    expected_sessions = []
    sessions_config = ue_config.get('sessions', [])
    if isinstance(sessions_config, list) and sessions_config:
        for sess in sessions_config:
            if not isinstance(sess, dict):
                continue
            apn = str(sess.get('apn', 'internet')).strip() or 'internet'
            pdn_type = pdn_type_to_int(str(sess.get('type', 'IPv4v6')))
            slice_info = sess.get('slice', {})
            if not isinstance(slice_info, dict):
                slice_info = {}
            sst = int(slice_info.get('sst', 1))
            sd = _normalize_sd(slice_info.get('sd', None))
            expected_sessions.append((sst, sd, apn, pdn_type))
    else:
        nssai = ue_config.get('configured-nssai', ue_config.get('default-nssai', [{'sst': 1}]))
        sst = 1
        sd = None
        if isinstance(nssai, list) and nssai:
            first = nssai[0]
            if isinstance(first, dict):
                sst = int(first.get('sst', 1))
                sd = _normalize_sd(first.get('sd', None))
        expected_sessions.append((sst, sd, 'internet', 3))

    expected_sessions.sort()

    profiles = []
    for i in range(count):
        current_imsi = str(base_imsi_int + i).zfill(imsi_length)
        profiles.append({
            'imsi': current_imsi,
            'security': {
                'k': key,
...
def _extract_db_sessions(subscriber):
    result = []
    slices = subscriber.get('slice', [])
    ...
    for slc in slices:
        ...
        try:
            ...
            sst = 1
        sd = _normalize_sd(slc.get('sd', None))
        sessions = slc.get('session', [])
        if not isinstance(sessions, list):
            continue
        for sess in sessions:
            if not isinstance(sess, dict):
                continue
            name = str(sess.get('name', 'internet')).strip() or 'internet'
            pdu_type = sess.get('type', 3)
            try:
                pdu_type = int(pdu_type)
            except (TypeError, ValueError):
                pdu_type = 3
            result.append((sst, sd, name, pdu_type))

    result.sort()
    return result


def _compare_subscriber(expected, subscriber):
    issues = []

    security = subscriber.get('security', {}) if isinstance(subscriber, dict) else {}
    db_k = _normalize_hex(security.get('k'))
    db_amf = _normalize_hex(security.get('amf'))
    db_op = _normalize_hex(security.get('op'))
    db_opc = _normalize_hex(security.get('opc'))

    exp_security = expected['security']
    exp_k = _normalize_hex(exp_security.get('k'))
    exp_amf = _normalize_hex(exp_security.get('amf'))
    exp_op = _normalize_hex(exp_security.get('op'))
    exp_opc = _normalize_hex(exp_security.get('opc'))

    if db_k != exp_k:
        issues.append(f"security.k mismatch (db={db_k}, expected={exp_k})")

    if db_amf != exp_amf:
        issues.append(f"security.amf mismatch (db={db_amf}, expected={exp_amf})")

    if exp_security.get('op_type') == 'OPC':
        if db_opc != exp_opc:
            issues.append(f"security.opc mismatch (db={db_opc}, expected={exp_opc})")
    else:
        if db_op != exp_op:
            issues.append(f"security.op mismatch (db={db_op}, expected={exp_op})")

    db_sessions = _extract_db_sessions(subscriber)
    exp_sessions = list(expected['sessions'])
    if db_sessions != exp_sessions:
        db_set = set(db_sessions)
        exp_set = set(exp_sessions)
        missing_sessions = sorted(exp_set - db_set)
        extra_sessions = sorted(db_set - exp_set)
        if missing_sessions:
            issues.append(f"missing sessions in DB: {missing_sessions}")
        if extra_sessions:
            issues.append(f"extra sessions in DB: {extra_sessions}")

    return issues


def check_multi_subscribers_from_yaml(db, yaml_file, count, imsi_override=None):
    """Check expected multi-UE subscribers from YAML against DB without modifying DB."""
    profiles = _extract_expected_multi_profiles(yaml_file, count, imsi_override=imsi_override)
    if profiles is None:
        return 2
    ...
    print(f"Checking {count} subscriber profile(s) from YAML '{yaml_file}'...")

    for profile in profiles:
        imsi = profile['imsi']
        subscriber = db.subscribers.find_one({'imsi': imsi})
        if subscriber is None:
            print(f"[MISSING] IMSI {imsi} not found in Open5GS DB")
            missing += 1
            continue

        issues = _compare_subscriber(profile, subscriber)
        if issues:
            incompatible += 1
            print(f"[INCOMPATIBLE] IMSI {imsi}")
            for issue in issues:
                print(f"  - {issue}")
        else:
            print(f"[OK] IMSI {imsi}")

    print(
        "Check complete: "
        f"checked={count}, missing={missing}, incompatible={incompatible}, "
        f"compatible={count - missing - incompatible}"
    )

    return 0 if (missing == 0 and incompatible == 0) else 3
...
def add_multi_subscribers_from_yaml(db, yaml_file, count):
    ...
    first_slice = True
    for (sst, sd), sessions in slice_sessions.items():
        base_slices_template.append(
            {
                'sst': sst,
                'sd': sd,
                'default_indicator': first_slice,
            ...
            base_slices_template.append(
                {
                    ...
                    'sessions_template': [create_session("internet", pdn_type=3)],
                }
            )
    ...
    for i in range(count):
        ...
        for tmpl in base_slices_template:
            ...
            slice_doc = create_slice(
                sst=tmpl['sst'],
                sd=tmpl['sd'],
                sessions=sessions,
                default_indicator=tmpl['default_indicator'],
            )
            slices.append(slice_doc)
        ...
        added_count += 1
        print(f"  Added IMSI {current_imsi}")
...
Commands:
  ...
  check_multi_yaml <yaml_file> <count>
                                      Check multi-UE subscribers from YAML against DB
                                      (warning only)
  ...
  lbo_roaming_allowed <imsi> <allowed>
    ...
    parser.add_argument('--imsi_override', default=None,
                        help='Override base IMSI/SUPI for *_multi_yaml commands')
    ...
    try:
        if cmd == 'add':
            ...
            try:
                ...
                sys.exit(1)
            if args.imsi_override:
                print("Warning: --imsi_override is ignored for add_multi_yaml")
            add_multi_subscribers_from_yaml(db, cmd_args[0], count)

        elif cmd == 'check_multi_yaml':
            if len(cmd_args) != 2:
                print("Usage: check_multi_yaml <yaml_file> <count>")
                sys.exit(1)
            try:
                count = int(cmd_args[1])
                if count < 1:
                ...
                print("Error: count must be a valid integer")
                sys.exit(1)
            code = check_multi_subscribers_from_yaml(
                db,
                cmd_args[0],
                count,
                imsi_override=args.imsi_override,
            )
            sys.exit(code)
