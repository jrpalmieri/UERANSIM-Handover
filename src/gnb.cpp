    ...
    if (yaml::HasField(config, "ntn"))
    {
        ...
        if (yaml::HasField(ntn, "timeWarp"))
        {
            ...
            if (yaml::HasField(timeWarp, "startEpoch"))
            {
                ...
                result->ntn.timeWarp.startEpochMillis = nr::sat::TleEpochToUnixMillis(startEpochText);
...
static void ReadOptions(int argc, char **argv)
{
    ...
    opt::OptionItem itemSatTime = {'s', "sat-time", "Configure satellite time clock settings, used for NTN simulations. Overrides config file settings. Allows for chained commands separated by commas," 
        " e.g. --sat-time start-epoch=23001.500,tick-scaling=2.0,run", "sat-time"};
    ...
    // Configure the sat time clock at startup.
    //   Overrides the config file settings
    //   Used by the simulator to coordinate time among gNBs and UEs
    ...
    if (opt.hasFlag(itemSatTime))
    {
        ...
        // parse subcommands by commas and then key=value pairs
        // e.g: --sat-time start-epoch=23001.500,tick-scaling=2.0,run
        ...
        while (!parseCompleted)
        {
            ...
            if (subString == "run")
            {
                g_refConfig->ntn.timeWarp.startState = nr::sat::ESatTimeState::Moving;
            }
            else if (subString == "pause")
            { 
                g_refConfig->ntn.timeWarp.startState = nr::sat::ESatTimeState::Paused;
            }
            else
            {
                ...
                if (key == "start-epoch")
                {
                    g_refConfig->ntn.timeWarp.startEpochMillis = nr::sat::TleEpochToUnixMillis(value);
                }
                else if (key == "tick-scaling")
                {
                    // parse into a double
                    g_refConfig->ntn.timeWarp.tickScaling = std::stod(value);
                    if (g_refConfig->ntn.timeWarp.tickScaling <= 0.0)
