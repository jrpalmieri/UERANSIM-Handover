...
# ======================================================================
#  Unit tests — ASN ConditionalReconfiguration builder
# ======================================================================

class TestChoAsnBuilder:
    """Verify ASN ConditionalReconfiguration payload generation helpers."""

    def test_conditional_reconfig_payload_add_mod_only(self):
        ...
        assert len(payload["condReconfigToAddModList"]) == 1
        item = payload["condReconfigToAddModList"][0]
        assert item["condReconfigId"] == 1
        assert item["condExecutionCond"] == [10, 11]
        assert item["condRRCReconfig"] == b"\x01\x02"

    def test_conditional_reconfig_payload_remove_only(self):
        codec = RrcCodec.__new__(RrcCodec)
        codec._asn1 = None

        payload = codec.build_conditional_reconfiguration_payload(
            candidates_to_add_mod=None,
            candidate_ids_to_remove=[1, 2, 8],
        )

        assert "condReconfigToAddModList" not in payload
        assert payload["condReconfigToRemoveList"] == [1, 2, 8]

    def test_conditional_reconfig_payload_add_and_remove(self):
        codec = RrcCodec.__new__(RrcCodec)
        codec._asn1 = None
        ...
        assert payload["condReconfigToRemoveList"] == [3]

    def test_conditional_reconfig_rrc_builder_returns_bytes_without_asn1(self):
        codec = RrcCodec.__new__(RrcCodec)
        codec._asn1 = None
...
class TestChoLogParsing:
    ...
    def test_conditional_reconfig_apply_summary(self):
        ue = self._ue_with_logs([
            "[rrc] ConditionalReconfiguration applied: removed=1 removeMiss=0 "
            "added=2 updated=1 skipped=0 activeCandidates=3",
        ])
        info = ue.parse_cho_info()
        ...
        assert info["active_candidates"] == 3

    def test_dl_cho_apply_summary(self):
        ue = self._ue_with_logs([
    ...
    def test_apply_summaries_accumulate(self):
        ue = self._ue_with_logs([
            "[rrc] ConditionalReconfiguration applied: removed=1 removeMiss=1 "
            "added=2 updated=1 skipped=3 activeCandidates=4",
            "[rrc] DL_CHO applied: total=2 added=1 updated=2 skipped=0 activeCandidates=5",
        ])
        info = ue.parse_cho_info()
        assert info["applied_removed"] == 1
        assert info["applied_remove_miss"] == 1
        assert info["applied_added"] == 3
        assert info["applied_updated"] == 3
        assert info["applied_skipped"] == 3
        assert info["active_candidates"] == 5

    def test_conditional_reconfig_remove_only_summary(self):
        ue = self._ue_with_logs([
            "[rrc] ConditionalReconfiguration applied: removed=2 removeMiss=0 activeCandidates=1",
        ])
        info = ue.parse_cho_info()
        assert info["applied_removed"] == 2
        assert info["applied_remove_miss"] == 0
        assert info["applied_added"] == 0
        assert info["applied_updated"] == 0
        assert info["applied_skipped"] == 0
        assert info["active_candidates"] == 1

    def test_conditional_reconfig_remove_miss_summary(self):
        ue = self._ue_with_logs([
            "[rrc] ConditionalReconfiguration applied: removed=0 removeMiss=3 "
            "added=0 updated=0 skipped=0 activeCandidates=2",
        ])
        info = ue.parse_cho_info()
        assert info["applied_removed"] == 0
        assert info["applied_remove_miss"] == 3
        assert info["applied_added"] == 0
        assert info["applied_updated"] == 0
        assert info["applied_skipped"] == 0
        assert info["active_candidates"] == 2
