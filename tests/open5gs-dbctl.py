...
def check_imsi(db, imsi):
    """Check if an IMSI exists in the database. Returns the IMSI if found, otherwise NOT_FOUND."""
...
Commands:
  ...
  check_imsi <imsi>                   Check if IMSI exists in database
        """
    )
    ...
    try:
        if cmd == 'add':
            ...
            if len(cmd_args) != 1:
                print("Usage: check_imsi <imsi>")
                sys.exit(1)
            result = check_imsi(db, cmd_args[0])
            print(result)
